...
import { Shield, Search, User, ArrowLeft, Camera } from 'lucide-react'
...
export default function VerifyPage() {
  ...
  const handleScan = (result: string) => {
    if (result) {
      // Extract account ID from scan result
      // Assuming format: https://verisite.io/verify/{account}
      const accountId = result.split('/').pop()
      if (accountId) {
        router.push(`/verify/${accountId}`)
      }
    }
  }
      ...
      <div className="container max-w-xl py-16 flex flex-col items-center">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <p className="text-lg text-muted-foreground">
            Enter an account address or scan a QR code to verify credentials
          </p>
          <div className="h-1 w-24 bg-primary my-4"></div>
        </div>

        {/* QR Scanner Mode toggle button */}
        <button
          onClick={() => setShowScanner(!showScanner)}
          className="mb-6 inline-flex items-center justify-center rounded-full bg-muted p-2.5 text-primary-foreground"
        >
          <Camera className="h-6 w-6 text-primary" />
          <span className="ml-2">{showScanner ? 'Enter Account Manually' : 'Scan QR Code'}</span>
        </button>
        ...
        <div className="mt-8 text-center text-sm text-muted-foreground">
          <p>You can also share and scan credential QR codes directly.</p>
        </div>
      </div>
    </div>
  )
